...
def create_tutorials(destination, silent = False):
    """
    Copies mercury.settrie tutorial notebooks to `destination`. A folder will be created inside
    destination, named 'settrie_tutorials'. The folder `destination` must exist.
    ...
    Examples:
        >>> # copy tutorials to /tmp/settrie_tutorials
        >>> from settrie import create_tutorials
    ...
    if not os.path.exists(src):
        src = src.replace('/src/settrie/tutorials', '/notebooks')
    ...
    dst = os.path.join(dst, 'settrie_tutorials')
